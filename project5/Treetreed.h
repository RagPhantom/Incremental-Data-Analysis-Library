...
class Treetreed{
    private:
        ...
        // --- Project 4 ---
        // A intermediate variable to compute the hash index
        int W{};


        // Hash table
        Datasets* HashTable[512] = {nullptr};

        // Status of the hash table
        std::string HashTableStatus[512];

        // --- Project 5 ---

        // Tuple to store edges
        // Tuple = {Series_Code, Threshold, Relation}
        using edges = std::set<std::tuple<std::string, double, std::string>>;

        // Key = country name (source); Value = {Key = country name (destination), Value = edges(Tuple)}
        std::unordered_map<Datasets*, std::unordered_map<Datasets*, edges>> AdjacencyList;

        // Arraylist to store countries that satisfy the mean and threshold relationship according to the relation
        std::vector<Datasets*> QualifiedCountries;
    ...
    public:
        ...
        int hashProject(std::string Country_Code);

        // Secondary Hash function
        int hashSekai();

        // Helper function to lookup the country code in the hash table
        // Returns the index of the country located in the HashTable
        int lookup(std::string Country_Code);

        // Lookup the country code in the hash table
        void LOOKUP_P4(std::string Country_Code);

        // Remove this country from the hash table
        void REMOVE_P4(std::string Country_Code);

        // Helper function to insert this country into the hash table
        void insert(Datasets *element);

        // Insert this country into the hash table
        void INSERT_P4(std::string Country_Code);

        // Helper function to sort the datasets array, used in CLEAN_P4()
        void MergeSort(Datasets *array[], int start, int end);

        // Helper function to sort the datasets array, used in CLEAN_P4()
        void merge(Datasets *array[], int start_left, int start_right, int end_right);
        ...
        void UPDATE_EDGES_P5(std::string Series_Code, double threshold, std::string relation);

        // Output all countrie names that are adjacent to the given country
        void ADJACENT_P5(std::string Country_Code);

        // Depth First helper function to determine if the two countries are connected by a path in the graph
        bool DFS(std::string Country_Code1, std::string Country_Code2);

        // Determines if the two countries are connected by a path in the graph
        void PATH_P5(std::string Country_Code1, std::string Country_Code2);

        // If the two countries are connected by an edge, list their relationships
        void RELATIONSHIPS_P5(std::string Country_Code1, std::string Country_Code2);
};