...
void TimeSeries::LOAD_P1(std::string line){
...
void TimeSeries::setLength(int len){
    length = len;
}
...
void TimeSeries::setSeriesName(std::string name){
    SeriesName = name;
}

void TimeSeries::setSeriesCode(std::string code){
    SeriesCode = code;
}

void TimeSeries::setData(int index, double value){
    if (index >= capacity){
        double_array_size();
    }
    data[index] = value;
...
void TimeSeries::setYear(int index, int value){
    if (index >= capacity){
        double_array_size();
    }
    year[index] = value;
...
double TimeSeries::getData(int index){
    return data[index];
}
